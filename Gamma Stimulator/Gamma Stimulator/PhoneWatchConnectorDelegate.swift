//
//  PhoneWatchConnectorDelegate.swift
//  Gamma Stimulator
//
...
class PhoneWatchConnector: NSObject, WCSessionDelegate {
    ...
    // Start haptic feedback on the watch
    func startWatchHapticFeedback() {
        guard let session = session, session.isReachable else {
            print("Watch is not reachable")
            return
        }
        
        let message = ["command": "startHaptic"]
        session.sendMessage(message, replyHandler: nil) { error in
            print("Error starting watch haptic: \(error.localizedDescription)")
        }
    }

    // Stop haptic feedback on the watch
    func stopWatchHapticFeedback() {
        guard let session = session, session.isReachable else {
            print("Watch is not reachable")
            return
        }
        
        let message = ["command": "stopHaptic"]
        session.sendMessage(message, replyHandler: nil) { error in
            print("Error stopping watch haptic: \(error.localizedDescription)")
        }
    }
